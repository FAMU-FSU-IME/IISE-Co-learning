...
class geneticalgorithm_inner():
    
    with open("gga.csv", "w", newline='') as ff:
    ...
    def __init__(self, function, dimension, variable_type='bool', \
        ...
        self.__name__=geneticalgorithm_inner
        #############################################################
        # input function
    ...
    def run(self):
        ...
        with open("gga.csv", "a", newline='') as fff:
            gga_writer = csv.writer(fff)
            old_cost = self.best_function
  
            solution_cov1=int(self.best_variable[0])
            solution_cov2=int(self.best_variable[1])
            solution_cov3=int(self.best_variable[2])
            gga_writer.writerow([-(old_cost), mcmc_sample_95[0][solution_cov1],mcmc_sample_95[1][solution_cov2],mcmc_sample_95[3][solution_cov3]])
        return (self.best_variable),(self.best_function)
##############################################################################         
##############################################################################         
