...
## estimate covariance based on the source data
# assume there are M samples in the source data
# assume every time we sample, there are n samples selected from the M samples and 1<=n<=S<=M
# for s = 1 to S
 #    sample s number of samples from the total M samples and repreat for C(s,M) times
#    use the s number of samples to estimate parameter  beta, gamma, rho with independent priors
...
coords_12 = []
width_12 = []
with open('S600F9000_ino120_1.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    for row in readCSV:
...
coords_14 = []
width_14 = []
with open('S600F3000_ino120_1.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    for row in readCSV:
...
coords_15 = []
width_15 = []
with open('S400F9000_ino120_1.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    for row in readCSV:
...
coords_16 = []
width_16 = []
with open('S400F3000_ino120_1.csv') as csvfile:
    readCSV = csv.reader(csvfile, delimiter=',')
    for row in readCSV:
...
##=============================================================================
#data = [copy.deepcopy(data_copy[0]), copy.deepcopy(data_copy[1]), copy.deepcopy(data_copy[2]), copy.deepcopy(data_copy[4]), copy.deepcopy(data_copy[5]), copy.deepcopy(data_copy[6]), copy.deepcopy(data_copy[7]), copy.deepcopy(data_copy[8]), copy.deepcopy(data_copy[9]),copy.deepcopy(data_copy[10])]
...
thres_list =  [0.8, 0.8, 0.8, 0.75, 0.75, 0.82, 0.8, 0.75, 0.82, 0.58, 0.76,0.76, 0.80, 0.9, 0.80,0.85,0.6083,0.5954,0.7564,0.7435,0.8026,0.5674,0.5547,0.5463,0.5421,0.5453]
...
# train a model between bp_ac and bp2, which represents the speed constant phase
...
####sampling --> parameter estimation--> calculate parameter covariance
...
##Select the sample you would like to co-learn with and only one g-code is selected from each pritner.
##Since ino120 is the target and contains only one g-code, so one of the g-code in ino120 needs to be chosen.
    
sample_training_list = [0,9]
...
# calcualte sample covariance of parameters ### For the comparison with the previous study.
def cov_cal(x, y):
    return np.sum((x-np.mean(x))*(y-np.mean(y)))/(len(x) - 1)
cov_1, cov_12, cov_2 = np.var(parameters[:, 4]), cov_cal(parameters[:, 4], parameters[:, 5]), np.var(parameters[:, 5])


print("covariance:",cov_1,cov_12,cov_2,)

cor = np.corrcoef(parameters[:, 4],parameters[:, 5])

print("correlation:",cor)

print("parameter 3:",parameters[:, 3])

print("parameter 4:",parameters[:, 4])

print("parameter 5:",parameters[:, 5])
plt.figure()
plt.scatter(parameters[:, 4],parameters[:, 5])
m, b = np.polyfit(parameters[:, 4], parameters[:, 5], 1)
plt.plot(parameters[:, 4], m*parameters[:, 4] + b,linestyle='dashed')
...
# define probability
p = 0.975
df = len(parameters[:,4])-2
# retrieve value <= probability
critical_value = t.ppf(p, df)
print("critical_value of Two-tailed test with Aplha 0.05 and df",df,"is: ",critical_value,"and",-critical_value )
# confirm with cdf
p = t.cdf(critical_value , df)
print("p_value of Two-tailed test with Aplha 0.05 and df",df,"is: ",(1-p)*2)


t_value = (cor*np.sqrt(len(parameters[:,4])-2))/(np.sqrt(1-cor**2))
print("t_value with Aplha 0.05 and df",df,"is: ",t_value[0][1])

if(np.abs(t_value[0][1])>=critical_value):
   
   
   
    print("The correlation between parameter 4 and 5 is significant with p value:",(1-t.cdf(t_value[0][1] , df))*2)
   
else:
    print("The correlation between parameter 4 and 5 is not significant with p value:",(1-t.cdf(t_value[0][1] , df))*2)
...
def MCMC_Covariance_Estimation(parameter_x1, parameter_x2, Time, Burn_in):
    ...
    row = 0
    hpd_count = 0
    fig, image = plt.subplots(2,4,figsize=(30,10))

   

    x = np.linspace(0.01, 1, 100)
    mu_0 = np.matrix([0,0])
    sigma_0 = np.eye(2,2)*10
    idt =idt + 1
    k_0 = 1
    df_0 = 6
    #iw_0 = invwishart.pdf(x, df=df_0, scale = sigma_0)
    #plt.plot(x, iw_0)
    #plt.figure()
    ...
    print('parameters_combined: ',parameters_combined)
    ...
    for j in range(0,2):
        for k in range(0,2):
            ...
            #Build the Highest Posterior Denssty
            theta_burn_in_95 =[]
           
            image[row,count].plot(range(Burn_in,Time+1), theta_burn_in, 'g-')
            image[row,count].set_xlabel("Iterations",fontsize=12)
            image[row,count].set_ylabel("θ",fontsize = 12)
            ...
            image[row,count+1].hist(theta_burn_in, num_bins, normed=1, facecolor='turquoise', alpha=0.5)
            
            Credible_95[count1] =copy.deepcopy(theta_burn_in_95)
            count1 = count1+1
            image[row,count+1].hist(theta_burn_in_95, num_bins, normed=1, facecolor='red', alpha=0.5)
            image[row,count+1].set_xlabel("theta",fontsize=12)
            image[row,count+1].set_ylabel("Density",fontsize=12)
            red_patch = mpatches.Patch(color='tomato', label='The narrowest 95% credible interval')
            image[row,count+1].legend(handles=[red_patch],fontsize = 12)
...
test = MCMC_Covariance_Estimation(4,5,12000,2000)
...
plt.figure()
plot_acf(MCMC_autocorrelation[0],lags=1000)

plot_acf(MCMC_autocorrelation[1],lags=1000)

plot_acf(MCMC_autocorrelation[2],lags=1000)

plot_acf(MCMC_autocorrelation[3],lags=1000)
plt.show()



with open("MCMC_diagonstics3.csv", "a", newline='') as f:
    writer = csv.writer(f)
    for row in MCMC_autocorrelation:
        writer.writerow(row)
        
#############################Cummulative autocorrelation plot###################################################################
##################Uncomment the part, when all the models are run#################################

#plt.figure(figsize=(8,5))      
#from statsmodels.tsa.stattools import acf
#MCMC_multiple = []
#with open("MCMC_diagonstics3.csv", "r", newline='') as rf:
#    reader = csv.reader(rf, delimiter=',')
#    for row in reader:
#        MCMC_multiple.append(list(np.array(row).astype(float)))
#                
#        if (acf(list(np.array(row).astype(float)),nlags = 1000)[200])<0.4:
#            plt.plot(acf(list(np.array(row).astype(float)),nlags = 1000))
